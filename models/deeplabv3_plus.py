    ...
    def __init__(
        ...
        if output_stride == 8:
            for n, m in self.layer3.named_modules():
                if "conv1" in n and (
                    backbone == "resnet34" or backbone == "resnet18"
                ):
...
class Block(nn.Module):
    def __init__(
        ...
        rep.append(
            SeparableConv2d(
                in_channels, out_channels, 3, stride=1, dilation=dilation
            )
        )
        ...
        rep.append(
            SeparableConv2d(
                out_channels, out_channels, 3, stride=1, dilation=dilation
            )
        )
        rep.append(nn.BatchNorm2d(out_channels))
...
class Xception(nn.Module):
    def __init__(self, output_stride=16, in_channels=3, pretrained=True):
        ...
        # Exit flow
        self.block20 = Block(
            728, 1024, stride=1, dilation=ef_d[0], exit_flow=True
        )
...
def assp_branch(
    in_channels, out_channles, kernel_size, dilation, use_resolution=False
):
...
class ASSP(nn.Module):
    def __init__(self, in_channels, output_stride, use_resolution=False):
        ...
        self.avg_pool = nn.Sequential(
            ...
            (Conv2d(in_channels, 256, 1, bias=False)),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
        )

        self.conv1 = (
            Conv2d(256 * 5, 256, 1, bias=False)
            if not use_resolution
            else ResolutionWithConv(256 * 5, 256, 1, bias=False)
        )
...
class Decoder(nn.Module):
    def __init__(self, low_level_channels, num_classes, use_resolution=False):
        ...
        self.conv1 = (
            Conv2d(low_level_channels, 48, 1, bias=False)
            if not use_resolution
            else ResolutionWithConv(low_level_channels, 48, 1, bias=False)
        )
        ...
        self.output = nn.Sequential(
            (
                Conv2d(48 + 256, 256, 3, stride=1, padding=1, bias=False)
                if not use_resolution
                else ResolutionWithConv(
                    48 + 256, 256, 3, stride=1, padding=1, bias=False
                )
            ),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            (
                ...
                if not use_resolution
                    ...
                    256, 256, 3, stride=1, padding=1, bias=False
                )
            ),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            ...
            (
                ...
                if not use_resolution
                else ResolutionWithConv(256, num_classes, 1, stride=1)
            ),
        )
        initialize_weights(self)
    ...
    def __init__(
        ...
        **_,
        ...
        self.decoder = Decoder(
            low_level_channels, num_classes, use_resolution=use_resolution
        )
